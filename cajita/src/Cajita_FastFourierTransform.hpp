...
template <class BackendType, class Scalar> struct HeffteBackendTraits {};
template <> struct HeffteBackendTraits<Kokkos::Cuda, double> {
  using backend_type = heffte::backend::cufft;
  using complex_type = cufftDoubleComplex;
};
template <> struct HeffteBackendTraits<Kokkos::Cuda, float> {
  using backend_type = heffte::backend::cufft;
  using complex_type = cufftComplex;
};
#endif
template <class Scalar> struct HeffteBackendTraits<Kokkos::OpenMP, Scalar> {
  using backend_type = heffte::backend::fftw;
  using complex_type = std::complex<Scalar>;
};
template <class Scalar> struct HeffteBackendTraits<Kokkos::Serial, Scalar> {
  using backend_type = heffte::backend::fftw;
  using complex_type = std::complex<Scalar>;
};
...
public:
  ...
  FastFourierTransform(const ArrayLayout<EntityType, MeshType> &layout) {
    ...
    std::array<int, 3> local_num_entity = {(int)entity_space.extent(Dim::K),
...
public:
  ...
  */
  HeffteFastFourierTransform(const ArrayLayout<EntityType, MeshType> &layout,
      ...
      : FastFourierTransform<Scalar, EntityType, MeshType, DeviceType>(layout) {
    if (1 != layout.dofsPerEntity())
      throw std::logic_error("Only 1 complex value per entity allowed in FFT");
    ...
    _fft = std::make_shared<heffte::fft3d<backend_type>>(
        inbox, outbox, layout.localGrid()->globalGrid().comm(), heffte_params);
    ...
    //_fft_work = Kokkos::View<Kokkos::complex<Scalar> *, DeviceType>(
  ...
  */
  template <class Array_t> void forward(const Array_t &x, const FFTScaleNone) {
    compute(x, 1, heffte::scale::none);
  }
  template <class Array_t> void forward(const Array_t &x, const FFTScaleFull) {
  ...
  */
  template <class Array_t> void reverse(const Array_t &x, const FFTScaleNone) {
    compute(x, -1, heffte::scale::none);
  }
  template <class Array_t> void reverse(const Array_t &x, const FFTScaleFull) {
    compute(x, -1, heffte::scale::full);
  }
  template <class Array_t>
  ...
  cufftComplex inline copyFromKokkosComplex(
      Kokkos::complex<value_type> x_view_val, cufftComplex work_view_val) {
    auto realpart = x_view_val.real();
    auto imagpart = x_view_val.imag();
    work_view_val.x = realpart;
    work_view_val.y = imagpart;
    return work_view_val;
  }

  cufftDoubleComplex inline copyFromKokkosComplex(
      Kokkos::complex<value_type> x_view_val,
      cufftDoubleComplex work_view_val) {
    auto realpart = x_view_val.real();
    auto imagpart = x_view_val.imag();
    work_view_val.x = realpart;
    work_view_val.y = imagpart;
  ...
  std::complex<value_type> inline copyFromKokkosComplex(
      Kokkos::complex<value_type> x_view_val,
      std::complex<value_type> work_view_val) {
    auto realpart = x_view_val.real();
    auto imagpart = x_view_val.imag();
    work_view_val.real(realpart);
    work_view_val.imag(imagpart);
    return work_view_val;
  }

  Kokkos::complex<value_type> inline copyToKokkosComplex(
      cufftComplex work_view_val, Kokkos::complex<value_type> x_view_val) {
    auto realpart = work_view_val.x;
    auto imagpart = work_view_val.y;
    x_view_val.real() = realpart;
    x_view_val.imag() = imagpart;
    return x_view_val;
  }
  ...
  Kokkos::complex<value_type> inline copyToKokkosComplex(
      std::complex<value_type> work_view_val,
      Kokkos::complex<value_type> x_view_val) {
    auto realpart = work_view_val.real();
    auto imagpart = work_view_val.imag();
    x_view_val.real() = realpart;
    x_view_val.imag() = imagpart;
    return x_view_val;
  }
  ...
  void compute(const Array_t &x, const int flag, const heffte::scale scale) {
    ...
    static_assert(std::is_same<typename Array_t::mesh_type, mesh_type>::value,
    ...
    auto work_view = createView<complex_type, Kokkos::LayoutRight, DeviceType>(
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const int i, const int j, const int k) {
          ...
          work_view(iw, jw, kw) =
              copyFromKokkosComplex(x_view(i, j, k, 0), work_view(iw, jw, kw));
        });
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const int i, const int j, const int k) {
          ...
          auto kw = k - own_space.min(Dim::K);
          x_view(i, j, k, 0) =
              copyToKokkosComplex(work_view(iw, jw, kw), x_view(i, j, k, 0));
        });
  }
